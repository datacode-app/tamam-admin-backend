...
# Step 3: Deploy Production Environment Template
echo ""
echo "🔧 Step 3: Deploying production environment template..."
rsync -avz --progress create-production-env.sh -e "ssh -i $SSH_KEY" root@$SERVER_IP:$REMOTE_PATH/
ssh -i $SSH_KEY root@$SERVER_IP "cd $REMOTE_PATH && chmod +x create-production-env.sh && ./create-production-env.sh"
echo "✅ Production environment template deployed"
echo "⚠️  IMPORTANT: Environment file created with placeholders - update with real credentials"
...
# Step 6.5: Verify module activation file and module status
echo ""
echo "🧪 Step 6.5: Verifying module activation (modules_statuses.json & module:list)..."
ssh -i $SSH_KEY root@$SERVER_IP "
    cd $REMOTE_PATH && 
