@php($safeCombinations = is_array($combinations) ? $combinations : (is_string($combinations) ? json_decode($combinations, true) : []))
@if(count($safeCombinations) > 0)
    <table class="table table-borderless table--vertical-middle">
        ...
        @foreach ($safeCombinations as $key => $combination)
            <tr>
