...
?>
{{-- {{ dd($countryCode) }} --}}
<html dir="{{ $site_direction }}" lang="{{ str_replace('_', '-', app()->getLocale()) }}"  class="{{ $site_direction === 'rtl'?'active':'' }} {{ in_array(app()->getLocale(), ['ar', 'ckb']) ? 'lang-'.app()->getLocale() : '' }}">
<head>
    <meta charset="utf-8">
    ...
    <!-- Vazirmatn Global Font for Arabic & Kurdish -->
    <link href="{{asset('assets/admin/css/vazirmatn-global.css')}}" rel="stylesheet">
    ...
    <!-- Additional CSS for RTL languages -->
    ...
    <style>
        ...
        /* Enhanced Arabic/Kurdish font rendering for vendor panel */
        .page-title, .card-title, .table th, .table td, 
        .nav-link, .btn, .form-label, .form-control,
        .sidebar-nav-menu .nav-link, .vendor-info {
            font-family: 'Vazirmatn', 'Segoe UI', Tahoma, Arial, sans-serif !important;
        }
...
<body class="footer-offset {{ in_array(app()->getLocale(), ['ar', 'ckb']) ? 'lang-'.app()->getLocale() : '' }}">
    @if (env('APP_MODE')=='demo')
...
<script>
    // Keep toasts on screen longer for better debugging visibility
    if (window.toastr) {
    ...
    if (typeof window.$ === 'undefined') {
        console.warn('jQuery not found before toastr initialization');
    }
    window.__TOAST_DEBUG_LONG_TIMEOUT__ = true;
</script>
