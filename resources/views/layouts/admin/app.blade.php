...
?>
<html dir="{{ session()->get('site_direction') }}" lang="{{ str_replace('_', '-', app()->getLocale()) }}" class="{{session()->get('site_direction') === 'rtl'?'active':'' }} {{ in_array(app()->getLocale(), ['ar', 'ckb']) ? 'lang-'.app()->getLocale() : '' }}">
<head>
    <meta charset="utf-8">
...
<script>
    // Ensure jQuery is available globally as $ for theme.min.js
...
<script>
    ...
    // Ensure jQuery is bound before any toast usage
    window.$ = window.jQuery = window.jQuery || window.$;
    if (typeof window.$ === 'undefined') {
        console.warn('jQuery not found before toastr initialization');
    }
    // Developer note: import flows emit multiple errors; longer timeout helps review them
