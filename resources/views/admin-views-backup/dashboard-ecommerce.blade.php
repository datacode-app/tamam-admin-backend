...
@push('script_2')
    ...
    <!-- Universal Bulk Import Widgets -->
    <div id="universal-bulk-import-container" class="mt-4" style="display: none;">
        <div class="row">
            <!-- Items Import -->
            <div class="col-12 mb-3">
                @include('admin-views.partials._universal-bulk-import-widget', [
                    'type' => 'items',
                    'route' => 'admin.item.bulk-import',
                    'title' => 'Item Bulk Import'
                ])
            </div>
        </div>
    </div>

    <!-- JavaScript for Universal Bulk Import -->
    <script>
        function loadInlineBulkImport(type) {
            const container = document.getElementById('universal-bulk-import-container');
            
            // Show the container
            container.style.display = 'block';
            container.scrollIntoView({ behavior: 'smooth', block: 'start' });
            
            // Load appropriate widget based on type
            switch(type) {
                case 'items':
                case 'stores':
                case 'categories':
                case 'addons':
                    // Universal widgets are already loaded, just show the container
                    console.log(`Universal bulk import widget loaded for: ${type}`);
                    break;
                default:
                    console.warn('Import type not supported:', type);
                    break;
            }
        }
