...
@push('script_2')
<script src="https://maps.googleapis.com/maps/api/js?key={{ \App\Models\BusinessSetting::where('key', 'map_api_key')->first()?->value ?? 'Not Set' }}&libraries=places&callback=initMap&v=3.49">
</script>
                                    ...
                                    service.getDistanceMatrix(request).then((response) => {
                                        ...
                                        if($store->sub_self_delivery ){
                                            ...
                                            if ($module_wise_delivery_charge) {
                                                ...
                                                $per_km_shipping_charge = (float)\App\Models\BusinessSetting::where(['key' => 'per_km_shipping_charge'])->first()?->value ?? 'Not Set';
                                                $minimum_shipping_charge = (float)\App\Models\BusinessSetting::where(['key' => 'minimum_shipping_charge'])->first()?->value ?? 'Not Set';
