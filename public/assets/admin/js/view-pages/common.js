...
// Language tabs: scope behavior to the nearest container to avoid cross-form interference
$(document).on('click', '.lang_link', function (e) {
    e.preventDefault();
    // Activate only within the current nav group
    const $nav = $(this).closest('.nav');
    $nav.find('.lang_link').removeClass('active');
    $(this).addClass('active');

    // Determine a local container (prefer the closest form, then card, else body)
    ...
    // Hide all language forms within this container only
    $container.find('.lang_form').addClass('d-none');

    // Derive language key from id (expected pattern: `${lang}-link`)
    const formId = this.id || '';
    const lang = formId.endsWith('-link') ? formId.slice(0, -5) : formId;

    // Show any matching blocks for this language (supports -form, -form1, -form2, ...)
    $container.find(`[id^="${lang}-form"]`).removeClass('d-none');

    // Also reveal any default-form blocks when 'default' tab is active
    if (lang === 'default') {
        $container.find('.default-form').removeClass('d-none');
    }
});
