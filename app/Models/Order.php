...
class Order extends Model
{
    ...
    protected $casts = [
        ...
        // 'business_driver_id' => 'integer', // TEMPORARILY DISABLED - Fleet Management Feature
    ];

    protected $appends = ['module_type','order_attachment_full_url','order_proof_full_url','delivery_by'];
    ...
    /**
     * Get delivery attribution - who handled the delivery
     * @return string
     */
    public function getDeliveryByAttribute()
    {
        ...
        // If no delivery man assigned and store has self-delivery
        if (is_null($this->delivery_man_id) && $this->store && $this->store->self_delivery_system) {
            return 'Store Self-Delivery';
        }
        
        // If delivery man assigned and store has self-delivery system
        if (!is_null($this->delivery_man_id) && $this->store && $this->store->self_delivery_system) {
            return 'Store Delivery';
        }
        
        // If delivery man assigned and store doesn't have self-delivery
        if (!is_null($this->delivery_man_id) && $this->store && !$this->store->self_delivery_system) {
            return 'Tamam Delivery';
        }
        
        // Default case - no delivery man, pickup order or store self-delivery
        return 'No Delivery / Pickup';
    }

    // FLEET MANAGEMENT METHOD - TEMPORARILY DISABLED
    // TODO: Re-enable after completing database migrations and proper testing
    
    // /**
    //  * Get the delivery attribution text for customer display.
    //  * 
    //  * @return string
    //  */
    // public function getDeliveryAttribution()
    // {
    //     // Load relationships if not already loaded
    //     if (!$this->relationLoaded('store')) {
    //         $this->load('store');
    //     }
    //     if (!$this->relationLoaded('businessDriver')) {
    //         $this->load('businessDriver');
    //     }

    //     // If using business delivery service
    //     if ($this->delivery_service_type === 'business' && $this->businessDriver && $this->store) {
    //         return "Delivered by " . $this->store->name;
    //     }
        
    //     // If using pre-set attribution name (from assignment)
    //     if ($this->delivery_attribution_name) {
    //         return $this->delivery_attribution_name;
    //     }
        
    //     // Default to Tamam delivery
    //     return "Delivered by Tamam";
    // }
    ...
    // FLEET MANAGEMENT RELATIONSHIP - TEMPORARILY DISABLED
    // TODO: Re-enable after completing database migrations and proper testing
    
    // /**
    //  * Get the business driver assigned to this order (if using business delivery).
    //  */
    // public function businessDriver()
    // {
    //     return $this->belongsTo(BusinessDeliveryFleet::class, 'business_driver_id');
    // }
