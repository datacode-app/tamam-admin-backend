...
class LoginController extends Controller
{
    ...
    public function login($login_url)
    {
        ...
        try {
            ...
            $language = null; // Fallback when database is not available
        }
        
        $lang = 'en';
        $direction = 'ltr';
        if ($language) {
            $languages = json_decode($language->value, true);
            $defaultFound = false;
            ...
            // If no default found, use first language
            if (!$defaultFound && !empty($languages)) {
        ...
        try {
            ...
            $data = []; // Fallback when database is not available
        }
    ...
    public function submit(Request $request)
    {
        ...
        // Skip CAPTCHA validation in debug mode
        ...
        if (in_array($request->role, ['admin', 'admin_employee'])) {
            $user = Admin::where('email', $request->email)->first();
            
            if (!$user) {
                return redirect()->back()->withInput($request->only('email', 'remember'))
                    ->withErrors(['email' => translate('messages.email_does_not_exists')]);
            }
            
            // Role validation: admin_employee should NOT have role_id 1 (that's for admin)
            if ($request->role === 'admin_employee' && $user->role_id === 1) {
                return redirect()->back()->withInput($request->only('email', 'remember'))
                    ->withErrors(['email' => translate('messages.invalid_role')]);
            }
            
            // Admin role should have role_id 1
            if ($request->role === 'admin' && $user->role_id !== 1) {
                return redirect()->back()->withInput($request->only('email', 'remember'))
                    ->withErrors(['email' => translate('messages.invalid_role')]);
            }
        }
        ...
        if ($data == 'admin') {
            ...
            // BUSINESS SETUP REDIRECT DISABLED - Always go to dashboard
