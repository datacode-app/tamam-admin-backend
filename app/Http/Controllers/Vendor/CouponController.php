...
class CouponController extends Controller
{
    ...
    {
        // 🛡️ BULLETPROOF TRANSLATION LOADING - Use trait method with additional where clause
        $coupon = Coupon::withoutGlobalScope('translate')
            ->where(['id' => $id])
            ->where('created_by', 'vendor')
            ->first();
            
        if ($coupon) {
            // Load translations using bulletproof method
            $coupon = $this->loadWithTranslations('App\Models\Coupon', $id);
        }
