...
class ItemController extends Controller
{
    ...
    public function edit(Request $request,$id)
    {
        ...
        if($request->temp_product){
            ...
            // 🛡️ BULLETPROOF TRANSLATION LOADING - Use trait method instead of failing eager loading
            if(isset($request->product_gellary) && $request->product_gellary == 1 ){
                $product = Item::withoutGlobalScope([StoreScope::class, 'translate'])
                    ->where('is_approved',1)
                    ->findOrFail($id);
                
                // Manually load translations using bulletproof method
                $product = $this->loadWithTranslations('App\Models\Item', $id);
            } else {
                $product = $this->loadWithTranslations('App\Models\Item', $id);
            }
            
            // 🚨 CRITICAL DEBUG: Log translation loading status
            \Log::info('🔍 Item Edit Debug for ID: ' . $id, [
                'translations_loaded' => $product->relationLoaded('translations'),
            ...
            // 🚨 CRITICAL FIX: Ensure translations are loaded and force array access to work
            if (!$product->relationLoaded('translations') || $product->translations->isEmpty()) {
                \Log::info('🚨 Loading translations manually for item ' . $id);
                $translations = \App\Models\Translation::where('translationable_type', 'App\\Models\\Item')
                    ->where('translationable_id', $id)
                    ->get();
                $product->setRelation('translations', $translations);
                \Log::info('✅ Manual translations loaded: ' . $translations->count());
            }
            
            // 🔧 DEBUG: Log each translation
            foreach($product->translations as $t) {
                \Log::info("Translation: {$t->locale} - {$t->key}: {$t->value}");
            }
            
            // 🔧 FORCE ARRAY ACCESS: Ensure $product['translations'] works in blade template
            // Laravel models should support array access, but let's explicitly set the attribute
            $product->setAttribute('translations', $product->translations);
        }
    ...
    public function list(Request $request)
    {
        ...
        // Fix: Add missing $toggle_veg_non_veg variable
    ...
    public function bulk_import_index(Request $request)
    {
        ...
        // Support template download via GET (mirrors Admin ItemController behavior)
        if ($request->has('download') && $request->download === 'template') {
            $multilingualService = new MultilingualImportService();
            return $this->downloadVendorItemTemplate($request, $multilingualService, $module_type);
        }
    ...
    public function bulk_import_data(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            \Log::error('Vendor Item bulk import file validation failed', [
                'errors' => $errors,
        ...
        try {
            ...
            \Log::info('Vendor Item bulk import: processing file', [
                'vendor_id' => Helpers::get_store_id(),
            ...
            \Log::info('Vendor Item bulk import: file imported', [
                'vendor_id' => Helpers::get_store_id(),
        ...
        if($request->button == 'import'){
            $data = [];
            $temp_data = [];
            $allTranslations = []; // Store all translations for bulk insert
            $itemCollectionMap = []; // Map item index to original collection data
            ...
            try{
                foreach ($collections as $key=> $collection) {
                    ...
                    if (empty($collection['Id']) && empty($collection['Name'])) {
                        continue;
                    }
                    ...
                    if (!empty($missingFields)) {
                        ...
                        $errorMessage .= ". Item ID: " . ($collection['Id'] ?? 'Unknown');
                        Toastr::error($errorMessage);
                        \Log::error('Vendor Item bulk import validation failed', [
                            'vendor_id' => Helpers::get_store_id(),
                            'row' => $rowNumber,
                            'missing_fields' => $missingFields,
                            'item_id' => $collection['Id'] ?? 'Unknown',
                            'row_data' => $collection
                        ]);
                        return back();
                    }
                    ...
                    if(isset($collection['Price']) && ($collection['Price'] < 0)) {
                        Toastr::error("Row {$rowNumber}: Price must be greater than 0. Item ID: " . $collection['Id']);
                        return back();
                    }
                    if(isset($collection['Discount']) && ($collection['Discount'] < 0)) {
                        Toastr::error("Row {$rowNumber}: Discount must be greater than or equal to 0. Item ID: " . $collection['Id']);
                        return back();
                    }
                    if(isset($collection['Discount']) && ($collection['Discount'] > 100)) {
                        Toastr::error("Row {$rowNumber}: Discount must be less than or equal to 100. Item ID: " . $collection['Id']);
                        return back();
                    }
                    if (data_get($collection,'Image') != "" &&  strlen(data_get($collection,'Image')) > 30 ) {
                        Toastr::error("Row {$rowNumber}: Image name must be 30 characters or less. Item ID: " . $collection['Id']);
                        return back();
                    }
                    // Only validate times if provided; otherwise, defaults will be applied below
                    if ($collection['AvailableTimeStarts'] !== '' || $collection['AvailableTimeEnds'] !== '') {
                        try{
                            $t1= Carbon::parse($collection['AvailableTimeStarts'] ?: '00:00:00');
                            $t2= Carbon::parse($collection['AvailableTimeEnds'] ?: '23:59:59');
                            if($t1->gt($t2)) {
                                Toastr::error("Row {$rowNumber}: Available Time End must be after Available Time Start. Item ID: " . $collection['Id']);
                                return back();
                            }
                        }catch(\Exception $e){
                            info(["line___{$e->getLine()}",$e->getMessage()]);
                            Toastr::error("Row {$rowNumber}: Invalid time format for Available Time. Use HH:MM:SS format. Item ID: " . $collection['Id']);
                            return back();
                        }
                    }
                    ...
                    // Store original collection data for multilingual processing
                    $itemCollectionMap[$key] = $collection;
            ...
            try{
                ...
                $itemIndexOffset = 0; // Track the current offset in original data array
                foreach($chunk_items as $chunkIndex => $chunk_item){
//                    DB::table('items')->insert($chunk_item);
                    foreach ($chunk_item as $itemIndex => $item) {
                        ...
                        $originalIndex = $itemIndexOffset + $itemIndex;
                        if (isset($itemCollectionMap[$originalIndex])) {
                            $translations = $multilingualService->processMultilingualData(
                                $itemCollectionMap[$originalIndex], 
                                'Item', 
                                $insertedId
                            );
                    ...
                    $itemIndexOffset += count($chunk_item);
                }
...
//                    DB::table('items')->upsert($chunk_item,['id','module_id'],['name','description','image','images','category_id','category_ids','unit_id','stock','price','discount','discount_type','available_time_starts','available_time_ends','variations','food_variations','add_ons','attributes','store_id','status','veg','recommended', 'updated_at','choice_options']);
    ...
    public function stock_limit_list(Request $request)
    {
        ...
        return view('vendor-views.product.stock_limit_list', compact('items', 'category', 'type', 'toggle_veg_non_veg'));
    ...
    /**
     * Download multilingual vendor item template
     * 
     * @param Request $request
     * @param MultilingualImportService $multilingualService
     * @param string $moduleType
     * @return \Symfony\Component\HttpFoundation\BinaryFileResponse
     */
    private function downloadVendorItemTemplate(Request $request, MultilingualImportService $multilingualService, string $moduleType)
    {
        $baseHeaders = [
        ...
        if ($isMultilingual) {
            $headers = $multilingualService->generateMultilingualHeaders('Item', $baseHeaders);
            $exampleData = $multilingualService->createExampleData('Item');
            
            // Create multilingual template with vendor-specific data
            $templateData = [];
            if (!empty($exampleData)) {
                foreach ($exampleData as $example) {
                    $templateData[] = array_merge([
                        ...
                        'DiscountType' => $example['discount_type'],
                        ...
                        'Status' => $example['status'] ? 'active' : 'inactive',
                        'Veg' => 'yes',
                        'Recommended' => 'no'
                    ], [
            ...
            $filename = 'Vendor_' . ucfirst($moduleType) . 'Items_multilingual_template.xlsx';
        } else {
            $headers = $baseHeaders;
            $templateData = [[
                'Id' => 1,
            ...
            $filename = 'Vendor_' . ucfirst($moduleType) . 'Items_template.xlsx';
        }
