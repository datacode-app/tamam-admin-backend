...
// use App\Models\BusinessDeliveryFleet; // TEMPORARILY DISABLED - Fleet Management Feature
...
class VendorController extends Controller
{
    ...
    // FLEET MANAGEMENT METHODS - TEMPORARILY DISABLED FOR STABLE RELEASE
    // TODO: Re-enable after completing database migrations and proper testing
    
    // /**
    //  * Get available drivers for order assignment.
    //  * SAFE: New endpoint, doesn't affect existing functionality.
    //  */
    // public function getAvailableDrivers(Request $request, $orderId)
    // {
    //     try {
    //         $vendor = $request['vendor'];
    //         
    ...
    //         })->with('store')->findOrFail($orderId);
    //         
    //         $store = $order->store;
    //         $drivers = [];
    //         
    //         // Get Tamam drivers if enabled (default behavior)
    //         if (in_array($store->delivery_service_type ?? 'tamam', ['tamam', 'both'])) {
    //             $tamamDrivers = DeliveryMan::available()
    //                 ->where('zone_id', $store->zone_id)
    //                 ->with(['rating'])
    //                 ->get()
    //                 ->map(function($driver) use ($store) {
    //                     return [
    //                         'id' => $driver->id,
    //                         'type' => 'tamam',
    ...
    //                         'phone' => $driver->phone ?? '',
    //                         'image' => $driver->image,
    //                         'avg_rating' => $driver->rating->avg('rating') ?? 5.0,
    //                         'order_count' => $driver->orders()->completed()->count(),
    //                         'distance' => $this->calculateDistance($store, $driver),
    //                         'vehicle_type' => $driver->vehicle ? $driver->vehicle->type : null,
    //                     ];
    //                 });
    //             
    //             $drivers = array_merge($drivers, $tamamDrivers->toArray());
    //         }
    //         
    //         // Get business drivers if enabled
    //         if (in_array($store->delivery_service_type ?? 'tamam', ['business', 'both'])) {
    //             $businessDrivers = $store->activeBusinessDrivers->map(function($driver) {
    //                 return [
    //                     'id' => $driver->id,
    //                     'type' => 'business',
    //                     'f_name' => $driver->driver_name,
    //                     'l_name' => '',
    //                     'phone' => $driver->driver_phone,
    //                     'image' => null,
    //                     'avg_rating' => 5.0, // Default for business drivers
    //                     'order_count' => $driver->completedOrdersCount,
    //                     'distance' => 0, // Business drivers are at the store
    //                     'vehicle_type' => $driver->vehicle_type,
    //                 ];
    //             });
    //             
    //             $drivers = array_merge($drivers, $businessDrivers->toArray());
    //         }
    //         
    //         // Sort drivers by preference (default: nearest)
    //         $preference = $store->preferred_driver_selection ?? 'nearest';
    //         $drivers = $this->sortDriversByPreference($drivers, $preference);
    //         
    //         return response()->json([
    //             'available_drivers' => $drivers,
    //             'delivery_service_type' => $store->delivery_service_type ?? 'tamam',
    //             'store_name' => $store->name
    //         ]);
    //         
    //     } catch (\Exception $e) {
    //         return response()->json([
    //             'errors' => [['code' => 'drivers', 'message' => 'Unable to fetch available drivers']]
    //         ], 500);
    //     }
    // }
    ...
    //  * SAFE: New endpoint, doesn't affect existing functionality.
    //  */
    // public function assignDriverToOrder(Request $request, $orderId)
    // {
    //     $validator = Validator::make($request->all(), [
    //         'delivery_man_id' => 'required|integer',
    //         'driver_type' => 'required|in:tamam,business',
    ...
    //         })->with('store')->findOrFail($orderId);
    //         
    //         if ($request->driver_type === 'tamam') {
    //             $driver = DeliveryMan::findOrFail($request->delivery_man_id);
    //             $order->delivery_man_id = $driver->id;
    //             $order->delivery_service_type = 'tamam';
    //             $order->delivery_attribution_name = 'Delivered by Tamam';
    //         } else {
    //             $businessDriver = BusinessDeliveryFleet::where('store_id', $order->store_id)
    //                 ->findOrFail($request->delivery_man_id);
    //             $order->business_driver_id = $businessDriver->id;
    //             $order->delivery_service_type = 'business';
    //             $order->delivery_attribution_name = 'Delivered by ' . $order->store->name;
    //         }
    //         
    ...
    //             'delivery_attribution' => $order->delivery_attribution_name
    //         ]);
    //         
    //     } catch (\Exception $e) {
    //         return response()->json([
    //             'errors' => [['code' => 'assignment', 'message' => 'Unable to assign driver']]
    //         ], 500);
    //     }
    // }

    // /**
    //  * Calculate distance between store and driver.
    //  * SAFE: Private helper method.
    //  */
    // private function calculateDistance($store, $driver)
    // {
    //     if (!$store->latitude || !$store->longitude || !$driver->latitude || !$driver->longitude) {
    //         return 0;
    //     }

    //     $earthRadius = 6371; // Earth's radius in kilometers
    //     
    //     $lat1 = deg2rad($store->latitude);
    //     $lon1 = deg2rad($store->longitude);
    //     $lat2 = deg2rad($driver->latitude);
    //     $lon2 = deg2rad($driver->longitude);
    //     
    //     $deltaLat = $lat2 - $lat1;
    //     $deltaLon = $lon2 - $lon1;
    //     
    ...
    // /**
    //  * Sort drivers by preference.
    //  * SAFE: Private helper method.
    //  */
    // private function sortDriversByPreference($drivers, $preference)
    // {
    //     switch ($preference) {
    //         case 'nearest':
    //             usort($drivers, fn($a, $b) => $a['distance'] <=> $b['distance']);
    //             break;
    //         case 'best_rated':
    //             usort($drivers, fn($a, $b) => $b['avg_rating'] <=> $a['avg_rating']);
    //             break;
    //         case 'least_busy':
    //             usort($drivers, fn($a, $b) => $a['order_count'] <=> $b['order_count']);
    //             break;
    //     }
    //     return $drivers;
    // }
