...
class OrderController extends Controller
{
    public function track_order(Request $request)
    {
        ...
        if ($order) {
            ...
            // Add optional delivery attribution - backward compatible
            $order['delivery_attribution'] = $order->getDeliveryAttribution();
...
//            'contact_person_number' => $request->contact_person_number ? ($request->user ? $request->contact_person_number :str_replace('+', '', $request->contact_person_number)) : ($request->user?$request->user->phone:''),
        ...
        // Auto-assign delivery man based on store configuration and order type
        $order->delivery_man_id = self::autoAssignDeliveryMan($order, $store, $request);
        
        // Increment assigned order count if delivery man was assigned
        if ($order->delivery_man_id) {
            DeliveryMan::where('id', $order->delivery_man_id)->increment('assigned_order_count');
        }
...
//        })
     ...
     * @return int|null
     */
    private static function autoAssignDeliveryMan($order, $store, $request)
    {
        // Skip assignment for pickup orders
        if ($request->order_type === 'take_away') {
            return null;
        }
        ...
        // For stores with self-delivery system, they handle their own delivery
        // even if the general 'delivery' flag is 0

        // If store has self-delivery system, try to assign store's delivery man
        if ($store->self_delivery_system == 1) {
            // Find available delivery man for this store
            $deliveryMan = DeliveryMan::where('store_id', $store->id)
                ->where('status', 1) // Active status
                ->where('active', 1) // Available
            ...
            // If no store delivery man available, don't assign Tamam delivery man
            // Let admin manually assign later
            return null;
        }

        // For non-self-delivery stores, check if general delivery is enabled
        if (!$store->delivery) {
            return null; // Store doesn't offer delivery at all
        }
        
        // Try to find available Tamam delivery man
        $deliveryMan = DeliveryMan::whereNull('store_id') // Tamam delivery men
            ->where('status', 1) // Active status
            ->where('active', 1) // Available
            ->whereHas('zone', function($query) use ($order) {
                $query->where('id', $order->zone_id);
            })
            ->orderBy('assigned_order_count', 'asc') // Least busy first
            ->first();

        return $deliveryMan ? $deliveryMan->id : null;
    }
}
