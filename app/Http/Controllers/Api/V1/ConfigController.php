...
//        }
            ...
            foreach ($system_languages as $language) {
                if (is_array($language) && isset($language['code'])) {
                    array_push($sys_lang_array, [
                        ...
                        'default' => $language['default'] ?? ($language['code'] === 'en')
                    ]);
                }
            }
        }
        ...
        if (is_array($social_login_settings)) {
            foreach ($social_login_settings as $social) {
                if (is_array($social) && isset($social['login_medium'], $social['status'])) {
                    $config = [
        ...
        if (isset($apples) && is_array($apples)) {
            foreach ($apples as $apple) {
                if (is_array($apple) && isset($apple['login_medium'], $apple['status'])) {
                    $config = [
        ...
        $digital_payment_infos = array(
            'digital_payment' => (boolean)((isset($digital_payment['status']) && $digital_payment['status'] == 1) ? true : false),
        ...
        return response()->json([
            ...
            'cash_on_delivery' => (boolean)((isset($cod['status']) && $cod['status'] == 1) ? true : false),
            'digital_payment' => (boolean)((isset($digital_payment['status']) && $digital_payment['status'] == 1) ? true : false),
            'digital_payment_info' => $digital_payment_infos,
            'per_km_shipping_charge' => (double)($settings['per_km_shipping_charge'] ?? 0),
            'minimum_shipping_charge' => (double)($settings['minimum_shipping_charge'] ?? 0),
            ...
            'schedule_order_slot_duration' => (int)($settings['schedule_order_slot_duration'] ?? 30),
