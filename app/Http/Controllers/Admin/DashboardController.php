...
class DashboardController extends Controller
{
    ...
    public function dashboard(Request $request)
    {
        ...
        // 🚀 PERFORMANCE OPTIMIZATION: Increase PHP execution time for dashboard
        set_time_limit(60);
    ...
    public function dashboard_data($request)
    {
        ...
        $top_customers = User::withCount('orders as order_count')
            ->when(is_numeric($params['zone_id']), function ($q) use ($params) {
                return $q->where('zone_id', $params['zone_id']);
            })
            ->having("order_count" , '>', 0)
        ...
        $top_restaurants = Store::withCount('orders as order_count')
            ->when(is_numeric($params['module_id']), function ($q) use ($params) {
        ...
        // 🚀 PERFORMANCE OPTIMIZATION: Cache data for 5 minutes
        cache()->put($cache_key, $dash_data, 300);
