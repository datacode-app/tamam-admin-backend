...
class VendorController extends Controller
{
    ...
    public function view(Request $request,$store_id, $tab=null, $sub_tab='cash')
    {
        ...
        if($tab == 'settings')
        {
            $toggle_veg_non_veg = \App\Models\BusinessSetting::where('key', 'toggle_veg_non_veg')->first()?->value ?? 0;
    ...
    {
        // Support template download with multilingual option
        if(request()->has('download')) {
            if(request()->get('type') === 'multilang') {
                return $this->download_multilang_template();
            }
            return $this->download_standard_template();
        }
    ...
    {
        // CRITICAL FIX: Enhanced file upload validation with detailed error messages
        $validator = \Validator::make($request->all(), [
        ...
        if ($validator->fails()) {
            ...
            \Log::error('Bulk import file validation failed', [
                'errors' => $errors,
        ...
        try {
            ...
            \Log::info('Processing bulk import file', [
            ...
            $collections = (new FastExcel)->import($storedAbsPath);
            
            \Log::info('File imported successfully', [
                'rows_count' => $collections->count(),
                'first_row_keys' => $collections->first() ? array_keys($collections->first()) : []
            ]);
            
        } catch (\Exception $exception) {
            \Log::error('FastExcel import failed', [
                'error' => $exception->getMessage(),
                'file' => $request->file('products_file')->getClientOriginalName(),
                'line' => $exception->getLine(),
                'trace' => $exception->getTraceAsString()
            ]);
            
            Toastr::error(translate('messages.file_import_error') . ': ' . $exception->getMessage());
            return back();
        }
        ...
        // CRITICAL FIX: Enhanced duplicate detection with specific row information
        if ($duplicate_emails->isNotEmpty()) {
            $duplicateEmailsList = $duplicate_emails->take(5)->implode(', ');
            \Log::error('Duplicate emails found in bulk import', [
                'duplicate_emails' => $duplicate_emails->toArray()
            ]);
            
            Toastr::error('Duplicate emails found in file: ' . $duplicateEmailsList . ($duplicate_emails->count() > 5 ? ' and ' . ($duplicate_emails->count() - 5) . ' more' : ''));
            return back();
        }

        if ($duplicate_phones->isNotEmpty()) {
            $duplicatePhonesList = $duplicate_phones->take(5)->implode(', ');
            \Log::error('Duplicate phones found in bulk import', [
                'duplicate_phones' => $duplicate_phones->toArray()
            ]);
            
            Toastr::error('Duplicate phone numbers found in file: ' . $duplicatePhonesList . ($duplicate_phones->count() > 5 ? ' and ' . ($duplicate_phones->count() - 5) . ' more' : ''));
            return back();
        }
        ...
        if($request->button == 'import'){
            ...
            // CRITICAL FIX: Enhanced database duplicate detection with specific information
            $existingStores = Store::whereIn('email', $email)->orWhereIn('phone', $phone)->get(['email', 'phone', 'name']);
            
            if($existingStores->isNotEmpty()) {
                $existingEmails = $existingStores->whereNotNull('email')->pluck('email')->take(3);
                $existingPhones = $existingStores->whereNotNull('phone')->pluck('phone')->take(3);
                
                \Log::error('Duplicate email/phone found in database', [
                    'existing_emails' => $existingEmails->toArray(),
                    'existing_phones' => $existingPhones->toArray(),
                    'total_conflicts' => $existingStores->count()
                ]);
                
                $errorMessage = 'Email/phone already exists in database: ';
                if ($existingEmails->isNotEmpty()) {
                    $errorMessage .= 'Emails(' . $existingEmails->implode(', ') . ')';
                }
                if ($existingPhones->isNotEmpty()) {
                    $errorMessage .= ' Phones(' . $existingPhones->implode(', ') . ')';
                }
            ...
            $stores = [];
            $emailToCollection = [];
            $allTranslations = []; // For multilingual support
            ...
            foreach ($collections as $key => $collection) {
                // FIXED: Validate zone_id and module_id before processing (Issue #8)
                if (isset($collection['zone_id']) && !Zone::find($collection['zone_id'])) {
                    Toastr::error(translate('messages.invalid_zone_id') . ': ' . $collection['zone_id']);
                    return back();
                }
                
                if (isset($collection['module_id']) && !Module::find($collection['module_id'])) {
                    Toastr::error(translate('messages.invalid_module_id') . ': ' . $collection['module_id']);
                    return back();
                }
                
                // CRITICAL FIX: Enhanced required field validation with specific error messages
                $requiredFields = [
                ...
                foreach ($requiredFields as $field => $displayName) {
                    if (!isset($collection[$field]) || $collection[$field] === "" || $collection[$field] === null) {
                        $missingFields[] = $displayName . " (Row " . ($key + 2) . ")";
                    }
                }
                
                if (!empty($missingFields)) {
                    \Log::error('Missing required fields in bulk import', [
                        'row' => $key + 2,
                        'missing_fields' => $missingFields,
                        'collection_keys' => array_keys($collection)
                    ]);
                    
                    Toastr::error('Missing required fields: ' . implode(', ', $missingFields));
                    return back();
                }
                // FIXED: Handle delivery time validation for "20-100 min" format (Issue #2)
                if(isset($collection['DeliveryTime'])) {
                    $delivery_time_str = trim($collection['DeliveryTime']);
                ...
                array_push($vendors, [
                    ...
                    // FIXED: Use client's requested default password 'Tamam@2025' instead of random
                    'password' => bcrypt('Tamam@2025'),
                ...
                $current_store_id = $store_id + $key + 1;
                ...
                array_push($stores, [
                    ...
                    // FIXED: Add support for missing columns (Issue #1)
                    'cutlery' => isset($collection['Cutlery']) ? ($collection['Cutlery'] == 1 || $collection['Cutlery'] == 'yes') ? 1 : 0 : 1,
                    // Note: DailyTime and ManageItemSetup are handled separately in schedule logic
                    'created_at' => now(),
                    'updated_at' => now(),
                ]);
                // Keep a map for translations by a stable key (email)
            ...
            try{
                ...
                $chunk_translations = !empty($translations) ? array_chunk($translations, $chunkSize) : [];
...
//                    DB::table('stores')->insert($chunk_store);
                    foreach ($chunk_store as $store) {
                        ...
                        // FIXED: Handle halal_tag_status in StoreConfig table (cutlery stays in stores table)
                        if (isset($emailToCollection[$store['email']])) {
                            $collection = $emailToCollection[$store['email']];
                            if (isset($collection['HalalTagStatus'])) {
                                $storeConfig = new StoreConfig();
                                $storeConfig->store_id = $insertedId;
                                $storeConfig->halal_tag_status = ($collection['HalalTagStatus'] == 1 || $collection['HalalTagStatus'] == 'yes') ? 1 : 0;
                                $storeConfig->save();
                            }
                        }

                        // Compute translations now using the real inserted ID via email mapping
                        if (isset($emailToCollection[$store['email']])) {
                            ...
                            $translations = $multilingualService->processMultilingualData($multilingualRow, 'Store', $insertedId);
                            $allTranslations = array_merge($allTranslations, $translations);
                        }
                    }
                }

                // Use the multilingual service to insert translations
                if (!empty($allTranslations)) {
                    $translationResult = $multilingualService->bulkInsertTranslations($allTranslations);
                    if (!$translationResult) {
                        // Log the issue but don't fail the entire import
                        \Log::warning('Multilingual translations failed to insert for bulk import', [
                            'translation_count' => count($allTranslations),
                            'stores_imported' => count($stores)
                        ]);
                        
                        // Show user-friendly message about multilingual data
                        Toastr::warning(translate('messages.stores_imported_but_translations_failed', ['count' => count($stores)]));
                    } else {
                        \Log::info('Successfully imported multilingual translations', [
                            'translation_count' => count($allTranslations),
                            'stores_imported' => count($stores)
                        ]);
                    }
            ...
            if ($translation_count > 0) {
                Toastr::success('Successfully imported ' . count($stores) . ' stores with ' . $translation_count . ' translations (Kurdish Sorani & Arabic)');
            } else {
...
//                    DB::table('stores')->upsert($chunk_store,['id','email','phone','vendor_id'],['name','logo','cover_photo','latitude','longitude','address','zone_id','module_id','minimum_order','comission','tax','delivery_time','minimum_shipping_charge','per_km_shipping_charge','maximum_shipping_charge','schedule_order','status','self_delivery_system','veg','non_veg','free_delivery','take_away','delivery','reviews_section','pos_system','active','featured']);
    ...
    private function download_standard_template()
    {
        $stores = Store::with(['vendor', 'zone', 'module'])->take(5)->get();

        $data = [];
        foreach ($stores as $store) {
            $data[] = [
                'ownerFirstName' => $store->vendor?->f_name ?? 'John',
                'ownerLastName' => $store->vendor?->l_name ?? 'Doe',
                'storeName' => $store->name,
                'phone' => $store->phone,
                'email' => $store->email,
                'logo' => $store->logo ?? 'def.png',
                'CoverPhoto' => $store->cover_photo ?? 'def.png',
                'latitude' => $store->latitude ?? '36.1916',
                'longitude' => $store->longitude ?? '44.0092',
                'Address' => $store->address ?? 'Erbil, Kurdistan',
                // Default if null: zone 10, module 2
                'zone_id' => $store->zone_id ?? 10,
                'module_id' => $store->module_id ?? 2,
                'MinimumOrderAmount' => $store->minimum_order ?? 0,
                'Comission' => $store->comission ?? 10,
                'Tax' => $store->tax ?? 5,
                'DeliveryTime' => $store->delivery_time ?? '30-40 min',
                'MinimumDeliveryFee' => $store->minimum_shipping_charge ?? 2,
                'PerKmDeliveryFee' => $store->per_km_shipping_charge ?? 1,
                'MaximumDeliveryFee' => $store->maximum_shipping_charge ?? 10,
                'ScheduleOrder' => $store->schedule_order ? 'yes' : 'no',
                'Status' => $store->status ? 'active' : 'inactive',
                'SelfDeliverySystem' => $store->self_delivery_system ? 'active' : 'inactive',
                'Veg' => $store->veg ? 'yes' : 'no',
                'NonVeg' => $store->non_veg ? 'yes' : 'no',
                'FreeDelivery' => $store->free_delivery ? 'yes' : 'no',
                'TakeAway' => $store->take_away ? 'yes' : 'no',
                'Delivery' => $store->delivery ? 'yes' : 'no',
                'ReviewsSection' => $store->reviews_section ? 'active' : 'inactive',
                'PosSystem' => $store->pos_system ? 'active' : 'inactive',
                'storeOpen' => $store->active ? 'yes' : 'no',
                'FeaturedStore' => $store->featured ? 'yes' : 'no',
                // FIXED: Add missing columns for client requirements (Issue #1)
                'HalalTagStatus' => $store->halal_tag_status ?? 1,
                'Cutlery' => $store->cutlery ?? 1,
                'DailyTime' => '09:00-22:00',
                'ManageItemSetup' => $store->schedule_order ? 'Enable' : 'Manual',
            ];
        }

        return (new FastExcel($data))->download('stores_bulk_format.xlsx');
    }

    private function download_multilang_template()
    {
        // Generate realistic multilingual template data with complete examples
        $data = [
            [
                ...
                'PerKmDeliveryFee' => 500,
                'MaximumDeliveryFee' => 10000,
                'ScheduleOrder' => 'yes',
                'Status' => 'active',
                'SelfDeliverySystem' => 'inactive',
                'Veg' => 'yes',
                'NonVeg' => 'yes',
                'FreeDelivery' => 'no',
                'TakeAway' => 'yes',
                'Delivery' => 'yes',
                'ReviewsSection' => 'active',
                'PosSystem' => 'active',
                'storeOpen' => 'yes',
                'FeaturedStore' => 'yes',
                // FIXED: Add missing columns for multilingual template (Issue #1)
                'HalalTagStatus' => 1,
                'Cutlery' => 1,
                'DailyTime' => '09:00-22:00',
            ...
            [
                ...
                'MinimumDeliveryFee' => 1500,
                'PerKmDeliveryFee' => 400,
                'MaximumDeliveryFee' => 8000,
                'ScheduleOrder' => 'yes',
                'Status' => 'active',
                'SelfDeliverySystem' => 'inactive',
                'Veg' => 'yes',
                ...
                'HalalTagStatus' => 1,
                'Cutlery' => 0,
                'DailyTime' => '08:00-20:00',
                'ManageItemSetup' => 'Manual',
            ],
            [
                'ownerFirstName' => 'Karwan',
                'ownerLastName' => 'Omar',
                'storeName' => 'Traditional Coffee House',
                'storeName_ku' => 'قاوەخانەی تەقلیدی',
                'storeName_ar' => 'مقهى تقليدي',
                'phone' => '+9647501234569',
                'email' => 'karwan.coffee@example.com',
                'logo' => 'def.png',
                'CoverPhoto' => 'def.png',
                'latitude' => '36.1920',
                'longitude' => '44.0096',
                'Address' => 'Near Citadel, Old Quarter',
                'Address_ku' => 'نزیک قەڵا، بەشی کۆن',
                'Address_ar' => 'قرب القلعة، الحي القديم',
                'zone_id' => 1,
                'module_id' => 2,
                ...
                'MaximumDeliveryFee' => 6000,
                'ScheduleOrder' => 'yes',
                'Status' => 'active',
                'SelfDeliverySystem' => 'active',
                'Veg' => 'yes',
                'NonVeg' => 'no',
                'FreeDelivery' => 'yes',
                'TakeAway' => 'yes',
                ...
                'HalalTagStatus' => 0,
                'Cutlery' => 1,
                'DailyTime' => '07:00-23:00',
                'ManageItemSetup' => 'Enable',
            ],
            [
                'ownerFirstName' => 'Shilan',
                'ownerLastName' => 'Mahmud',
                'storeName' => 'Slemani Fresh Market',
                'storeName_ku' => 'بازاڕی تازەی سلێمانی',
                'storeName_ar' => 'سوق السليمانية الطازج',
                'phone' => '+9647501234570',
                'email' => 'shilan.market@example.com',
                'logo' => 'def.png',
                'CoverPhoto' => 'def.png',
                'latitude' => '36.1922',
                'longitude' => '44.0098',
                'Address' => 'Main Bazaar Street, Sulaymaniyah',
                'Address_ku' => 'شەقامی بازاڕی سەرەکی، سلێمانی',
                'Address_ar' => 'شارع السوق الرئيسي، السليمانية',
                'zone_id' => 2,
                'module_id' => 3,
                ...
                'MinimumDeliveryFee' => 3000,
                'PerKmDeliveryFee' => 600,
                'MaximumDeliveryFee' => 12000,
                'ScheduleOrder' => 'yes',
                'Status' => 'active',
                'SelfDeliverySystem' => 'inactive',
                'Veg' => 'yes',
                ...
                'HalalTagStatus' => 1,
                'Cutlery' => 1,
                'DailyTime' => '10:00-22:00',
                'ManageItemSetup' => 'Manual',
            ],
            [
                'ownerFirstName' => 'Helin',
                'ownerLastName' => 'Rashid',
                'storeName' => 'Duhok Pharmacy Plus',
                'storeName_ku' => 'دەرمانخانەی دهۆک پڵەس',
                'storeName_ar' => 'صيدلية دهوك بلس',
                'phone' => '+9647501234571',
                'email' => 'helin.pharmacy@example.com',
                'logo' => 'def.png',
                'CoverPhoto' => 'def.png',
                'latitude' => '36.8623',
                'longitude' => '42.9988',
                'Address' => 'Medical Center, Duhok City',
                'Address_ku' => 'سەنتەری پزیشکی، شاری دهۆک',
                'Address_ar' => 'المركز الطبي، مدينة دهوك',
                'zone_id' => 3,
                'module_id' => 4,
        ...
        return (new FastExcel($data))->download('stores_multilang_template.xlsx');
    }
    ...
    /**
     * BULLETPROOF MULTILINGUAL STORE IMPORT - PERMANENT SOLUTION
     * 
     * This method replaces all the problematic legacy import code with a bulletproof system.
     * It prevents ALL recurring issues by using comprehensive validation, atomic transactions,
     * and self-healing mechanisms.
     * 
     * USE THIS METHOD for all store imports instead of the legacy bulk_import_data method.
     */
    public function bulletproof_store_import(Request $request)
    {
        // Simplify: delegate to the existing simple importer
        $request->merge(['button' => 'import']);
     ...
     */
    private function convertToUserFriendlyError(string $technicalError): string
    {
        // Comprehensive error message mapping
        $errorMappings = [
            'vendor_id' => '🔧 Database configuration error. Please contact your administrator.',
            'translationable_type' => '🌐 Translation system error. The import was rolled back safely. Please try again.',
            'Excel validation failed' => '📊 Your Excel file format is invalid. Please check the template and required columns.',
            'No data rows found' => '📄 Your Excel file is empty. Please add data rows and try again.',
            'Required column missing' => '📋 Missing required columns in your Excel file. Please use the correct template.',
            'Invalid email format' => '📧 Some email addresses in your Excel file are invalid. Please fix them and try again.',
            'Connection refused' => '🔌 Database connection error. Please try again in a few minutes.',
            'Duplicate entry' => '🔄 Some stores already exist in the system. Please check for duplicates.',
            'File not found' => '📁 Excel file could not be processed. Please try uploading again.',
            'Memory limit' => '💾 Excel file is too large. Please split it into smaller files.',
            'Timeout' => '⏱️ Import is taking too long. Please try with a smaller file.',
        ];

        // Find matching error pattern
        foreach ($errorMappings as $keyword => $message) {
            if (str_contains(strtolower($technicalError), strtolower($keyword))) {
                return $message;
            }
        }

        // Default user-friendly message
        return '❌ Import failed. Please check your Excel file format and try again. If the problem persists, contact support.';
    }

    /**
     * GET BULLETPROOF IMPORT STATUS
     * Returns the status of the last bulletproof import operation
     */
    public function getBulletproofImportStatus()
    {
        $importResult = session()->get('bulletproof_import_result');
        
        if (!$importResult) {
            return response()->json(['status' => 'no_recent_import']);
        }
        ...
        return response()->json([
            ...
            'summary' => $importResult['success'] ? [
                'imported_count' => $importResult['result']['imported_count'] ?? 0,
                'translations_verified' => $importResult['verification']['translations_verified'] ?? 0,
                'audit_entries' => count($importResult['audit_log'] ?? [])
            ] : [
