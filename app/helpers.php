...
if (! function_exists('translate')) {
    ...
    {
        // If the translator is not yet bound (e.g., during early exception rendering),
        // avoid calling trans() to prevent "Target class [translator] does not exist".
        if (!app()->bound('translator')) {
            $rawKey = strpos($key, 'messages.') === 0 ? substr($key, 9) : $key;
            $processedKey = ucfirst(str_replace('_', ' ', \App\CentralLogics\Helpers::remove_invalid_charcaters($rawKey)));
            return $processedKey;
        }
        ...
        $local = \App\Utils\KurdishLanguageHelper::normalizeKurdishToBackend($local);
