...
//        }
    ...
    {
        // Cache for 5 minutes to prevent database query on every request
        return \Illuminate\Support\Facades\Cache::remember('view_keys_cache', 300, function () {
    ...
    public static function add_or_update_translations($request, $key_data,$name_field ,$model_name, $data_id,$data_value , $model_class = false){
        try{
            // 🚨 DEBUG: Log all incoming data for vendor translation debugging
            \Log::info('🔍 Translation Save Debug:', [
                'key_data' => $key_data,
                'name_field' => $name_field,
                'model_name' => $model_name,
                'data_id' => $data_id,
                'data_value' => $data_value,
                'lang_array' => $request->lang ?? 'NO LANG ARRAY',
                'field_values' => $request->{$name_field} ?? 'NO FIELD VALUES'
            ]);
            ...
            foreach ($request->lang as $index => $key) {
                $field_value = $request->{$name_field}[$index] ?? null;
                
                // 🚨 DEBUG: Log each language processing
                \Log::info("Processing language: {$key} (index {$index})", [
                    'is_default_lang' => $default_lang == $key,
                    'field_value' => $field_value,
                    'has_value' => !empty($field_value)
                ]);
                
                // 🔧 FIXED LOGIC: Save translation if we have a value and it's not 'default'
                if ($field_value && $key != 'default') {
                    \Log::info("✅ SAVING translation for {$key}: {$field_value}");
                    ...
                    \Log::info("❌ NOT SAVING translation for {$key}: field_value={$field_value}, key={$key}");
                }
            }
            return true;
        } catch(\Exception $e){
    ...
    public static function get_full_url($path,$data,$type,$placeholder = null){
        $place_holders = [
            ...
            'business' => asset('storage/business/tamam-default-logo.png'),
            ...
            'business_logo' => asset('storage/business/tamam-default-logo.png'),
